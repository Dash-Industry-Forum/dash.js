...
dash.DashParser.prototype = {
	...
	parseRepresentations: function(items,parent) {
		...
		for(var i=0;i<len;i++)
		{
			...
			if(node) item.segmentBase = this.parseSegmentBase(node,parent.segmentBase); else item.segmentBase = parent.segmentBase;
			node = xml.querySelector(xml.nodeName + " > SegmentList");
			if(node) item.segmentList = this.parseSegmentList(node,parent.segmentList); else item.segmentList = parent.segmentList;
			node = xml.querySelector(xml.nodeName + " > SegmentTemplate");
			if(node) item.segmentTemplate = this.parseSegmentTemplate(node,parent.segmentTemplate); else item.segmentTemplate = parent.segmentTemplate;
