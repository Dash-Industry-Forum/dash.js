...
// First bytes of a real WebM file: EBML header, then a Segment element. No
// moof or mfhd anywhere, which is the case getSamplesInfo used to crash on.
const webmHeader = [
    ...
    0x42, 0x85, 0x81, 0x02, 0x18, 0x53, 0x80, 0x67, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x15, 0xca
];
...
describe('BoxParser', function () {
    ...
    describe('when the data is not ISOBMFF', () => {
        it('should return an empty samplesInfo for a WebM buffer instead of throwing', () => {
            const samplesInfo = boxParser.getSamplesInfo(new Uint8Array(webmHeader).buffer);

            expect(samplesInfo.sampleList).to.be.instanceOf(Array);
            expect(samplesInfo.sampleList).to.be.empty;
            expect(samplesInfo.lastSequenceNumber).to.be.NaN;
        });
    });
});
